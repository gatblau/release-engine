...
sequenceDiagram
  ...
  Portal->>API: POST /v1/jobs {tenant_id, path_key, params, idempotency_key, callback_url?, schedule?, first_run_at?, max_attempts?}
  API->>DB: First-write path: reserve idempotency key, then create jobs + jobs_read atomically
  DB-->>API: {job_id, state='queued', attempt=0, next_run_at}
  API-->>Portal: 202 Accepted
```

```sql
-- Step 1 (reserve): reserve idempotency key for this canonical request payload.
-- payload_fingerprint includes path_key, params, callback_url?, schedule?, and first_run_at? when provided.
INSERT INTO idempotency_keys (
  tenant_id, path_key, idempotency_key, payload_fingerprint,
  status_code, response_body_json, job_id, accepted_at, expires_at
)
VALUES (
  $tenant, $path, $idem, $fingerprint,
  202, $envelope_json, $job_id, now(), now() + interval '48 hours'
)
ON CONFLICT (tenant_id, path_key, idempotency_key) DO NOTHING;

-- Step 2 (first-write path only): create primary and read projection rows.
INSERT INTO jobs (
  id, tenant_id, path_key, idempotency_key, params_json, schedule,
  state, attempt, max_attempts, next_run_at, accepted_at, created_at, updated_at
)
VALUES (
  $job_id, $tenant, $path, $idem, $params, $schedule,
  'queued', 0, $max_attempts, coalesce($first_run_at, now()), now(), now(), now()
);
...
VALUES (
  $job_id, $tenant, $path, 'queued', 0, $max_attempts, $schedule, NULL, $run_id,
  NULL, coalesce($first_run_at, now()), now(), NULL, NULL,
  NULL, NULL, now(), now()
);

-- Step 3 (replay path): read existing idempotency row and compare payload_fingerprint.
-- Equal fingerprint => replay the stored canonical envelope (status fixed at first write).
-- Mismatch => 409 ERR_IDEM_CONFLICT with canonical envelope in the conflict payload.
```
...
- Periodic jobs are not claimed via a separate flow; recurrence is represented only by re-queuing with a future `next_run_at`, and this same claim query picks them up when due.
...
sequenceDiagram
  ...
  Runner->>DB: SELECT schedule, now() WHERE id=$id AND run_id=$runId AND state='running'
  Runner->>Runner: compute next_run_at if schedule is set
  Runner->>DB: UPDATE jobs SET state=succeeded OR queued(requeue) WHERE id=$id AND run_id=$runId AND state='running'
...
```sql
-- Read schedule and DB time under the same fence key.
SELECT schedule, now()
FROM jobs
WHERE id = $id AND run_id = $run_id AND state = 'running';

-- If schedule is null/empty: terminal success
UPDATE jobs
...
-- If schedule is set: success-requeue (non-terminal)
UPDATE jobs
SET state = 'queued', owner_id = NULL, lease_expires_at = NULL,
...
`OutboxDispatcher` maintains an explicit event-type registry for approval lifecycle events. The following types are built-in and must be present at startup:
...
- Services can only enqueue registered types.
- Unregistered types are rejected before persistence to prevent ambiguous downstream contracts.
- Approval events are emitted by `ApprovalService`/`ApprovalWorker` in the same logical state-change unit.
...
Approval lifecycle telemetry (Phase 8):
...
- `GET /v1/dora/summary` always includes non-null data quality fields in response payload.
- Deployment Frequency is sourced from `dora_deployment_frequency_daily`.
- Lead Time (`lead_time_p50_seconds`, `lead_time_p95_seconds`) is computed from `dora_commit_deployment_links` correlated with commit signals (`commit.pushed`, `commit.merged`).
- Webhook ingest failures return structured `200 OK` responses and are persisted to `dora_webhook_dead_letter` for triage/replay.

Instrumentation points:
- `ApprovalService.SeedStep()` increments `re_approval_requests_total` when a step enters `waiting_approval`.
- `ApprovalService.SubmitDecision()` increments `re_approval_decisions_total` and observes `re_approval_latency_seconds`.
- `ApprovalService.TickApprovals()` increments `re_approval_escalations_total` and `re_approval_timeouts_total`.
- `ApprovalWorker.Tick()` observes `re_approval_worker_tick_duration_seconds` for each polling cycle.
...
- **Standard operations**: `scm.create_repo`, `scm.protect_branch`, `cicd.create_pipeline`, `cloud.write_stack_definition`, `catalog.register_entity`

> **Note on `cloud.write_stack_definition`**: This operation writes Crossplane `XR`/`Claim` YAML definitions into a Git repository (via the GitHub connector) as a Kubernetes manifest. It does **not** call Crossplane or the Kubernetes API directly. ArgoCD subsequently detects the Git commit and applies the manifests to the cluster, whereupon Crossplane's controllers reconcile the declared cloud infrastructure. See the *GitOps Delivery Pattern* section in `d00.md` for the full delivery chain.
...
### 16.4 Secret Context and Requirements Interface
...
Each module implementation owns its tenant resolution logic. For example:

- **Infra module** — always uses platform tenant for infrastructure operations
- **Scaffold module** — derives tenant from customer inputs for service scaffolding
- **Promotion module** — may use different tenant contexts based on source and target environments

The module validates its own tenant context. If invalid inputs are provided, the module rejects them during construction — the runtime doesn't need to know validation rules.
...
#### 16.4.1 Physical-to-Logical Key Resolution
...
1. Module provides `TenantID` via `SecretContext()`
2. Connector declares required secrets via `RequiredSecrets(operation)`
3. Runtime maps logical→physical keys: `tenants/{TenantID}/{logicalKey}`
4. Volta executes scoped callback with decrypted secrets
5. Runtime remaps physical keys back to logical keys for connector consumption
6. Connector executes with logical secret map

This architecture ensures:
- **Module-owned tenant resolution**: Modules determine whose secrets to use
- **Connector-declared requirements**: Connectors declare what they need
- **Secure scoped execution**: Secrets exist only within Volta's guarded callback
- **No secret persistence**: Plaintext credentials never escape the scoped execution
...
#### 18.3.1 Approval Guardrails

Approval guardrails are evaluated at decision time for `POST /v1/jobs/{job_id}/steps/{step_id}/decisions`.

- **Self-approval**: deny when `self_approval=false` and `approver == job_owner`.
- **Role requirement**: deny when approver role is not in `allowed_roles`.
- **Tenant scope**: deny when approver tenant differs from job tenant.
- **Budget authority (optional)**: deny when `estimated_cost > approver_limit`.
- **Four-eyes progression**: step remains `waiting_approval` until `min_approvers` distinct approvals are recorded.

Decision-time contract:

- `PolicyEngine.EvaluateApproval()` determines whether a single decision is authorised.
- Four-eyes progression is evaluated after an authorised decision is persisted.
- Rejected decisions transition the step to `error` immediately.
- Approved decisions only transition to `running` when `approved_count >= min_approvers`.

Policy data source for approval guardrails is the path approval configuration and step approval context:
...
paths:
  deploy-production:
    approval:
      required: true
      min_approvers: 2
      allowed_roles: ["release-manager", "team-lead"]
      self_approval: false
      ttl: "30m"
      escalation_at: 0.8
```

#### 18.3.2 Approval Lifecycle (TTL, Escalation, Expiry)

Approval wait states are bounded and enforced by an internal worker loop.

- Every `waiting_approval` step has an `approval_expires_at` value.
- TTL resolution order:
  1. Step-level override (if provided by module request context)
  2. Path policy default (`approval.ttl`)
  3. System default (`48h`)
- Unbounded approval waits are not permitted.

Escalation behaviour:

- A step is escalated once elapsed time crosses `escalation_at * ttl`.
- Default escalation threshold is `0.8` (80% of TTL).
- Escalation is informational only and does not change step status.
- Emit outbox event `approval_escalated` with elapsed/remaining timing context.

Expiry behaviour:

- On expiry (`now() >= approval_expires_at`), transition the step to `error` with reason `approval_timeout`.
- Insert an `approval_decisions` record with `decision='expired'`, `approver='system'`.
- Emit outbox event `approval_expired`.

`ApprovalWorker` execution model:

- Poll interval: every `30s` (configurable).
- Query target set: steps where `status='waiting_approval'` and `approval_expires_at` is set.
- For each candidate, apply escalation and expiry rules in order.
- Use database time (`now()`) for all TTL comparisons.
...
### 20.1 DORA Backstage Consumption (Phase 2)

- Backstage DORA widgets consume:
  - `GET /v1/dora/summary`
  - `GET /v1/dora/deployments`
  - `POST /v1/webhooks/dora/{provider}` for inbound commit/incident signals routed through DORA normalizers
- Authorization context is claim-driven and must include exactly one scope selector per request:
  - `brand_id` (validated against JWT `brand_ids`), or
  - `group_id` (validated against JWT `group_ids` and `dora_group_brand_map` freshness).
- Group-map failures are fail-closed (`503`, `code=group_map_stale`) to avoid accidental over-exposure.
- Operational triage panels use:
