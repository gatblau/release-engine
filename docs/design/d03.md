...
Periodic scheduling does not introduce a separate claim mechanism. Recurring jobs are re-queued with a future `next_run_at`, and the same claimer path picks them up when `next_run_at <= now()`.
...
Claim eligibility is constrained to:
- queued jobs with `next_run_at <= now()`
- running jobs with expired lease (`lease_expires_at <= now()`) for reclaim
- jobs not currently blocked by unresolved `waiting_approval` steps
