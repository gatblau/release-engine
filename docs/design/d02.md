...
stateDiagram-v2
  ...
  running --> succeeded: 3a. finalise success (unscheduled)
  running --> queued: 3b. finalise success (scheduled requeue)
...
3a. **finalise success (unscheduled)** (`running → succeeded`): If `schedule` is null/empty, the worker completes all steps and finalises terminal success. Finalization is fenced by `WHERE run_id = $run_id AND state = 'running'`; a 0-row result means the lease was lost and the worker discards the result.

3b. **finalise success (scheduled requeue)** (`running → queued`): If `schedule` is set, success is non-terminal. The worker computes the next occurrence from the cron expression and re-queues the job with `next_run_at = <next occurrence>`, clearing lease ownership fields. This write is fenced identically by `run_id`.
...
type Module interface {
    ...
    // Query answers domain-specific questions about the resources the module manages.
    // This transforms the Release Engine from a pure execution plane to an
    // execution + query plane, where each module owns both its workflows AND
    // the read path for its domain.
    Query(ctx context.Context, query QueryRequest) QueryResult

    // Describe returns a self-description of the module's capabilities,
    // enabling discovery by agents, UIs, and other consumers.
...
// QueryRequest represents a domain-specific query to a module
type QueryRequest struct {
    Name   string                 `json:"name"`   // query name, e.g., "list-resources", "resource-health"
    Params map[string]any         `json:"params"` // query parameters
}

// QueryResult represents the result of a module query
type QueryResult struct {
    Status  string                 `json:"status"`  // "ok", "error"
    Data    map[string]any         `json:"data"`    // query result data
    Error   string                 `json:"error,omitempty"` // error message if status is "error"
}

// ModuleDescriptor provides self-description of a module's capabilities
type ModuleDescriptor struct {
    Name        string                   `json:"name"`
    Domain      string                   `json:"domain"` // "infrastructure", "deployment", "observability"
    ...
    EntityTypes []EntityTypeDescriptor   `json:"entityTypes"`
}

// OperationDescriptor describes an executable operation
type OperationDescriptor struct {
    Name              string                    `json:"name"` // "provision", "deprovision", "resize"
    Params            map[string]ParamSchema    `json:"params"`
    RequiresApproval  bool                      `json:"requiresApproval"`
}

// QueryDescriptor describes a query the module can answer
type QueryDescriptor struct {
    Name            string                    `json:"name"` // "list-resources", "resource-health", "drift-report"
    Params          map[string]ParamSchema    `json:"params"`
    ResponseSchema  string                    `json:"responseSchema"` // JSON Schema reference
}

// EntityTypeDescriptor describes an entity type the module manages
type EntityTypeDescriptor struct {
    Kind        string            `json:"kind"` // "rds-instance", "s3-bucket"
    Attributes  map[string]string `json:"attributes"` // available fields and their descriptions
}

// ParamSchema describes a parameter for operations or queries
...
## 6) External Effect Lifecycle with Secret Injection

> States and transitions for external effects (connector calls and webhooks) including secure secret injection via Volta scoped execution.
