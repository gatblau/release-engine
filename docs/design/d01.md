...
10. Client Portal queries module metadata via HTTP API with JWT authentication
...
flowchart LR
  ...
  API -->|4 Query Routing| QueryHandler
  API -->|5 Module Discovery| ModuleDiscovery
  API -->|6 Persist Job| DB
  API -->|7 Export Metrics| Metrics
  API -->|8 Trace Spans| Trace

  Scheduler -->|9 Fetch Runnable| DB
  Scheduler -->|10 Dispatch Job| Runner
  Runner -->|11 Resolve Module| Registry
  Runner -->|12 Execute Module| Modules
  Runner -->|13 Record Effect| DB
  Runner -->|14 Export Metrics| Metrics
  Runner -->|15 Trace Spans| Trace

  QueryHandler -->|16 Route Query| Modules
  ModuleDiscovery -->|17 Discover Modules| Registry
  Modules -->|18 Call Connector| Connectors
  Runner -->|19 Decrypt credential| Volta
  Connectors -->|20 Execute Call| Provider
  Provider -->|21 Return Result| Connectors

  Outbox -->|22 Persist Event| DB
  Outbox -->|23 Deliver Webhook| Callback
  Outbox -->|24 Export Metrics| Metrics
  Outbox -->|25 DLQ Escalation| DLQ

  Reconciler -->|26 Scan unknown_outcome| DB
  Reconciler -->|27 Query State| Provider

  Volta -->|"28 Bootstrap passphrase (startup)"| SM
  Volta -->|"29 Vault data (read/write)"| S3
```
...
- **Query Handler**: Handles HTTP requests for module queries (`GET /v1/query/{module}/{query}`). Routes queries to the appropriate module's `Query()` method and returns the results. Implements rate limiting, authentication, and authorization for query operations.

- **Module Discovery**: Manages module capability discovery via the `Describe()` method. Serves the `/v1/modules` and `/v1/modules/{module}/describe` endpoints, enabling agents, UIs, and other consumers to discover what operations and queries each module supports.
...
*Note: This diagram focuses on the job execution flow. For query flows (module discovery and domain queries), see the Component Architecture diagram in §2, which shows Query Handler and Module Discovery components handling API Layer → Query routing and module discovery.*
