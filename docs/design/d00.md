...
The **Release Engine** is the idempotent job scheduler at the heart of the Agentic TechOps platform. It provides the execution backbone that makes the entire platform trustworthy: every action — whether triggered by a human operator, an AI agent, or an automated policy — is expressed as a durable, observable, and concurrency-safe job. This design ensures that operations are predictable, auditable, and recoverable, regardless of their origin.
...
flowchart LR
    ...
    RUNNER     -->|"⑳ trigger delivery"| ARGO
    RUNNER     -->|"㉑ trigger provision"| CROSSPLANE
...
| ⑳ | Runner → ArgoCD | On successful completion of application-delivery steps, the Runner **notifies ArgoCD** to sync the target application from Git. |
| ㉑ | Runner → Crossplane | On successful completion of infrastructure steps, the Runner **notifies Crossplane** to reconcile the declared cloud compositions. |
| ㉒ | ArgoCD → Git | ArgoCD **pulls desired application state** (Helm charts, Kubernetes manifests) from Git as the single source of truth. |
| ㉓ | Crossplane → Git | Crossplane **pulls infrastructure compositions** from Git to drive cloud resource reconciliation via provider CRDs. |
| ㉔ | Git → EKS | ArgoCD applies manifests to **EKS clusters**, deploying workloads across one or more regions. |
| ㉕ | Git → Data Services | Data-service configurations (schema migrations, parameter groups) are **applied from Git**. |
| ㉖ | Crossplane → Cloud | Crossplane continuously **reconciles live cloud resources** (VPCs, RDS instances, MSK topics, etc.) against their declared compositions. |
