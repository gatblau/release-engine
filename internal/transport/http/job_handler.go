...
func NewJobHandler() *JobHandler {
	return newJobHandler(nil, nil)
}

func NewJobHandlerWithPool(pool db.Pool) *JobHandler {
	return newJobHandler(pool, nil)
}

func NewJobHandlerWithApprovalService(approvalService *ApprovalService) *JobHandler {
	return newJobHandler(nil, approvalService)
}

func newJobHandler(pool db.Pool, approvalService *ApprovalService) *JobHandler {
...
func (h *JobHandler) CreateJob(c echo.Context) error {
	...
	if req.Schedule != "" {
		if err := validateSchedule(req.Schedule); err != nil {
			return c.JSON(http.StatusBadRequest, map[string]string{"error_code": "ERR_INVALID_SCHEDULE"})
		}
	}
